{foreach item=value from=$plugin_index name=cnt}    
	<div class="tpl_blog_title">
        ...
        <div class="ttl">
        	<div class="date">
        		<div class="md">{$value.day}.{$value.month}</div>
        		<div class="year">{$value.year}</div>
        	</div>
        	<div class="right">
	        	<div class="title">
	            	<a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;sec={$value.sname}&amp;cat={$value.cname}&amp;year={$value.year}&amp;month={$value.month}&amp;title={$value.name}" title="{$value.title|CleanTitleAtr}">{$value.title}</a>
	        	</div>
