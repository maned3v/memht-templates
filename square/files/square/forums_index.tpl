		...
		<table cellpadding="0" cellspacing="0" border="0" width="100%">
			...
			{foreach item=category from=$plugin_forums name=cnt}
				...
				{if isset($category.forums)}
					{foreach item=forum from=$category.forums name=cntf}
						<tbody>
									...
									{if isset($forum.subforums) && sizeof($forum.subforums)>0}
										<div class="subforum_title">
											...
											{foreach item=subforum from=$forum.subforums name=cntsf}
												<div><img src="images/core/bullet.png" alt="{$subforum.id}" /><a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;f={$subforum.name}" title="{$subforum.title}">{$subforum.title}</a></div>
											{/foreach}
										</div>
									{/if}
								</td>
