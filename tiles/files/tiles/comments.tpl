{if $comments.info.status=='active'}
	{if $comments.info.comments>0}
        ...
        <div class="tpl_comments_item" style="background-color:#CCE8A8; text-align:center;">{$comments.info.retinfo}</div>
        {/if}
        {foreach item=value from=$comments.data name=com}
            {if $value.isadmin==1}
            <div id="comment{$value.id}" class="tpl_comments_item_h" style="background-color: {cycle values='#CCE8A8,#DDF0C6'};">
            {else}
            <div id="comment{$value.id}" class="tpl_comments_item" style="background-color: {cycle values='#CCE8A8,#DDF0C6'};">
            {/if}
                ...
                <div class="tpl_comments_text">{$value.text}</div>
            </div>
        {/foreach}
    ...
    {/if}
    <div class="tpl_comments_formbody">
    {if $comments.info.canwrite>0}
        ...
        <div>
			...
			<br /><textarea name="message" rows="5" cols="40" style="width:99%; height:150px;" class="bbcode"></textarea></div>
        <div>
