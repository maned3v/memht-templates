		...
		<div class="tpl_forums_post_ring">
			<div class="tpl_forums_post">
				<div>
					...
					<div class="bubble">{$post.userdata.posts} {t 1=POSTS} - <a name="post{$post.id}">#post{$post.id}</a></div>
					<div style="clear:both;"></div>
				</div>
				<div>
					<div class="avatar">{$post.userdata.avatar}</div>
					<div class="info">
						<div><a href="index.php?{$smarty.const._NODE}=user&amp;op=info&amp;uid={$post.userdata.uid}">{$post.userdata.options.stylename}</a><br />{$post.userdata.options.rolename}<br /><span>{$post.created}</span></div>
					</div>
					<div style="clear:both;"></div>
				</div>
				<div class="body">
