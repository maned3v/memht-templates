<div class="tpl_blog_title">
    ...
    <div class="tpl_blog_tools">
        ...
        <div style="text-align:right;"><a href="javascript:void(0);" onclick="javascript:openPopup('{$sys_site_url}/index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;op=printer&amp;title={$plugin_blog_view.name}','600','400')" title="{t 1=PRINT}" rel="tooltip"><img src="templates/{$sys_template}/images/printer.gif" alt="{t 1=PRINT}" /></a> <a href="javascript:void(0);" onclick="javascript:openPopup('{$sys_site_url}/index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;op=pdf&amp;title={$plugin_blog_view.name}','600','400')" title="{t 1=PDF}" rel="tooltip"><img src="templates/{$sys_template}/images/pdf.gif" alt="{t 1=PDF}" /></a> <a href="javascript:void(0);" onclick="javascript:openPopup('{$sys_site_url}/index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;op=rss&amp;cat={$plugin_blog_view.cname}','600','400')" title="{t 1=RSS}" rel="tooltip"><img src="templates/{$sys_template}/images/rss.gif" alt="{t 1=RSS}" /></a> <a href="javascript:void(0);" onclick="javascript:openPopup('{$sys_site_url}/index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;op=email&amp;title={$plugin_blog_view.name}','600','400')" title="{t 1=EMAIL}" rel="tooltip"><img src="templates/{$sys_template}/images/email.gif" alt="{t 1=EMAIL}" /></a></div>
	</div>
...
{if $related.info.status=="active" && isset($related.data) && sizeof($related.data)>0}
    ...
    <div class="tpl_related_box">
		{foreach item=value from=$related.data name=rel}
        	<div class="tpl_related_item"><img src="templates/{$sys_template}/images/out.png" width="13" height="11" alt="{$value.title|CleanTitleAtr}" title="{$value.title|CleanTitleAtr}" />&nbsp;<a href="{$value.url}" title="{$value.title|CleanTitleAtr}">{$value.title}</a></div>
        {/foreach}
	</div>
{/if}

<div class="tpl_social_title">{t 1=SHARE}</div>
<div class="tpl_social_box">
