{if isset($plugin_sec) && !empty($plugin_sec)}
	...
	{foreach item=value from=$plugin_sec name=cnt}
		{if isset($value.file) && !empty($value.file)}
			{if $smarty.foreach.cnt.iteration is div by 2}
			<div class="tpl_gallery_list_right">
			{else}
			<div class="tpl_gallery_list">
			{/if}
				<div class="tpl_gallery_element">{$value.title}</div>
				<a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;sec={$value.name}" title="{$value.title|CleanTitleAtr}"><img src="assets/gallery/sections/{$value.file}" alt="{$value.title|CleanTitleAtr}" title="{$value.title|CleanTitleAtr}" style="margin-bottom:0; float:none;" /></a>
			</div>
...
{if isset($plugin_cat) && !empty($plugin_cat)}
	...
	{foreach item=value from=$plugin_cat name=cnt}
		{if isset($value.file) && !empty($value.file)}
			...
			{/if}
				<div class="tpl_gallery_element">{$value.title}</div>
				<a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;sec={$value.sname}&amp;cat={$value.name}" title="{$value.title|CleanTitleAtr}"><img src="assets/gallery/categories/{$value.file}" alt="{$value.title|CleanTitleAtr}" title="{$value.title|CleanTitleAtr}" style="margin-top:0; float:none;" /></a>
			</div>
	...
	{foreach item=value from=$plugin_index name=cnt}
		{if isset($value.thumb) && !empty($value.thumb)}
			{if isset($plugin_opt.lightbox) && $plugin_opt.lightbox>0}
				...
				{/if}
					<div class="tpl_gallery_element">{$value.title}</div>
					<a href="assets/gallery/images/{$value.file}" class="gallery" rel="gallery" title="{$value.desc|CleanTitleAtr}{if $value.usecomments>0}<a href=&quot;index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;sec={$value.sname}&amp;cat={$value.cname}&amp;title={$value.name}&quot;>{t 1=COMMENTS}</a>{/if}"><img src="assets/gallery/images/{$value.thumb}" alt="{$value.id}" title="{$value.title|CleanTitleAtr}" style="margin-top:0; float:none;" /></a>
				</div>
			{else}
				<a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;sec={$value.sname}&amp;cat={$value.cname}&amp;title={$value.name}" title="{$value.title|CleanTitleAtr}"><img src="assets/gallery/images/{$value.thumb}" alt="{$value.title|CleanTitleAtr}" title="{$value.title|CleanTitleAtr}" style="margin-top:0; float:none;" /></a>
			{/if}
		{/if}
	{/foreach}
	</div>
